...
def publisher_detail(request, publisher_id):
    ...
    return render(
        ...
        {"publisher": publisher, "articles": articles},
    )
...
@login_required
@role_required(User.Role.JOURNALIST)
def article_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Assign the logged-in journalist before validation.
            article.author = request.user
            article.approved = False

            # Validate the complete article, including author/publisher.
            try:
                article.full_clean()
...
def newsletter_create(request):
    ...
    if request.method == "POST":
        ...
        # Assign the author before form validation.
        # Newsletter.clean() needs the author to exist.
...
def approve_article(request, article_id):
    """Approve an article and notify its author by email."""
...
class ArticleDetailAPI(APIView):
    ...
    def put(self, request, article_id):
        ...
        return self._update(request, article_id, partial=False)

    def patch(self, request, article_id):
        """Partially update an article owned by the journalist."""
        return self._update(request, article_id, partial=True)
...
def subscriptions(request):
    ...
    if request.method == "POST":
        subscribed_publishers = request.POST.getlist(
            "publishers"
        )
        subscribed_journalists = request.POST.getlist(
            "journalists"
        )
        ...
        messages.success(
            request,
            "Your subscriptions have been updated successfully.",
        )
